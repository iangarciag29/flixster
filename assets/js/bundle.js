...
// EXTERNAL MODULE: ./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/dist/cjs.js!./src/css/style.css
...
;// CONCATENATED MODULE: ./src/ts/buttons/goToTop.ts
const toggleBtn = document.getElementById("scrollToTopBtn");
const enableGoToTopBtn = () => {
...
;// CONCATENATED MODULE: ./src/ts/types/app.types.ts
var MOVIE_CATEGORIES;
(function (MOVIE_CATEGORIES) {
    MOVIE_CATEGORIES["TOP_RATED"] = "top_rated";
    MOVIE_CATEGORIES["UPCOMING"] = "upcoming";
    MOVIE_CATEGORIES["NOW_PLAYING"] = "now_playing";
    MOVIE_CATEGORIES["POPULAR"] = "popular";
})(MOVIE_CATEGORIES || (MOVIE_CATEGORIES = {}));
const stringToCat = (category) => {
    switch (category) {
        case "top_rated":
...
;// CONCATENATED MODULE: ./src/ts/handlers/MovieHandler.ts
...
class MovieHandler {
    constructor() {
        this.movies = [];
        this.category = MOVIE_CATEGORIES.UPCOMING;
        this.page = 1;
        this.limit = 30;
        this.total = -1;
        ...
        this.handleInputType = (e) => {
            ...
            if (movieSearchInput.value.length < 3)
                return;
        ...
        this.fetchMoviesByCategory = (page, category) => {
            const promise = axios_default().get(`${API_URL}/movie/${category}?api_key=${API_KEY}&language=en-US&page=${page}`);
            return promise.then(response => response.data);
        };
        this.searchMovies = (query, page) => {
            const promise = axios_default().get(`${API_URL}/search/movie?api_key=${API_KEY}&language=en-US&page=${page}&query=${query}&include_adult=false`);
            return promise.then(response => response.data);
        };
        this.endlessScroll = () => {
        ...
        this.hasMoreMovies = (page, limit, total) => {
       ...
       `;
        ...
        this.getMovieVideo = (id) => {
            const promise = axios_default().get(`${API_URL}/movie/${id}/videos?api_key=${API_KEY}`);
            return promise.then(response => response.data);
        };
        ...
        `;
                micromodal_es.show("movie-details");
            });
        };
